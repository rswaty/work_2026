
Linked to: [[2_LANDFIRE Communication]] 

Tags:  #objective2 #FSC


- [ ] update FSC online book #objective2 
	- [x] I need to package .tif and shape.  Combine with Stacey's ppkg into zipped folder 
	- [x] fold in Keith's images and language for section 8.
	- [ ] Stacey will do sections 3 and 5
	- [ ] I need to update section 4 about landfire data and downloading, esp 4.3.  
	- [ ] Me rebuild
	- [ ] Ask Mary Kelly or Sam Kurkowski to look

## Dashboard
- [ ] FSC: build ecoregions dashboard (https://rswaty.github.io/ecological_section_dash/dashboard.html) #objective2 
	- [ ] need to add in succussion class information
	- [ ] for succession classes
		- [ ] add instructions
	- [ ] add table
	- [ ] add bps descriptions
	- [ ] redo GIS with new sections https://data-usfs.hub.arcgis.com/datasets/50edb69f438b4ee5afa894758c88d18f_3/explore?location=36.964800%2C-95.859900%2C4

## From call with Matt Silveira, June 8, 2026




### Meeting with Carly Evans and Amy Clark Eagle, April 15, 2026
- Carly is part of the technical team. Dealing with standards and guidance; based on Atlanta.  More behind the scenes.  Complements outward facing
- People want tools to help with transition, esp with RSAs
- Ideas
	- update current online book
		- excited about this
	- parameterized reports; per ecoregion 
		- excited about this
		- me-would want design and technical help
		- levels of current protection
		- CRCS
		- they have a favorite ecoregions dataset.
		- RSAs
			- look in management.  what were native ecosystems?   what were they at the ecoregional scale.  How well are they protected?  For underrepresented or underprotected, go back to management scale
	- QGIS
	- LANDFIRE 101
- Many overlapping circles
	- Rickie White at NatureServe--small data help, now more deeply like Insight tool  Ricky proposed Ecoregion Scale change analysis.  Ricky is more of a forest management dude. Helped design insight
	- NatureServe, SFI, FSC, etc etc NatureServe have conversed around species and ecosystem data more accessible.  

- [x] get Stacey and Keith on updating the book!  How hard to include generating level of protection?  PAD.  #objective1
- [ ] meet Carly and Amy in Wisconsin April 27-May 1.  Try to meet Friday.  I am welcome on Wednesday workshop.  Thursday there is a field trip.  May meet on Friday.
- [x] Let Stuart know.  Who from TNC is going?  He is not going in person.  
- [x] Get ecoregions from Carly.  
- [x] Think through FSC dashboard #objective1

## Meeting with Carly on ecoregions, April 23, 2026

- USFS sections from 2017
- Native Ecosystems---Group Level of the NVC.  Will need to add in language!!  
- Protected areas super important
	- PAD-US data
	- 
